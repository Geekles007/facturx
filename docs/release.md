...
5. Publier : `pnpm publish --filter @geekles/facturx --access public` (depuis la racine ; `--dry-run` d'abord si doute).
...
7. Vérifier : `npm view @geekles/facturx version` et `npx -y -p @geekles/facturx@x.y.z node -e "console.log(require('@geekles/facturx/package.json').version)"`.
